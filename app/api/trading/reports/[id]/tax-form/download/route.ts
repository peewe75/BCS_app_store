...
import { getBlob, getTextBlob, buildTaxFormDraftKey, buildTaxFormFacsimilePdfKey } from '@/src/apps/trading/storage'
...
export async function GET(
  ...
  const blobKey = record?.facsimilePdfBlobKey ?? buildTaxFormFacsimilePdfKey(userId, reportId)
  const pdfData = await getBlob(blobKey)

  if (!pdfData) {
    return NextResponse.json({ error: 'PDF facsimile non ancora generato.' }, { status: 404 })
  }

  const filename = `facsimile-rw-rt-${report.filename?.replace(/\.(htm|html)$/i, '') ?? reportId}.pdf`
